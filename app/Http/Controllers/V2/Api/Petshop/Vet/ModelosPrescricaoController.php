...
class ModelosPrescricaoController extends Controller
{
    public function index(Request $request)
    {
        ...
        $query = ModeloPrescricao::query()
            ...
            ->when($request->filled('category'), function ($q) use ($request) {
                ...
                $categoriasMap = PrescriptionModelOptions::categories(); // key => label
            ...
            ->when($request->filled('status'), function ($q) use ($request) {
                ...
                if (! in_array($status, PrescriptionModelOptions::statuses(), true)) return;
        ...
        return response()->json([
            'data' => $data->getCollection()->map(fn (ModeloPrescricao $m) => $this->toV2Payload($m))->values(),
            'meta' => [
    ...
    {
        return response()->json([
            'categories' => array_values(PrescriptionModelOptions::categories()),
            'status' => collect(PrescriptionModelOptions::statusOptions())
                ->map(fn (string $label, string $value) => ['value' => $value, 'label' => $label])
                ->values()
                ->all(),
            'fieldTypes' => collect(PrescriptionModelOptions::fieldTypes())
                ->map(fn (string $type) => ['value' => $type, 'label' => PrescriptionModelOptions::fieldTypeLabel($type)])
                ->values()
                ->all(),
            'templates' => [
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $m = ModeloPrescricao::create([
                'empresa_id' => $empresaId,
                'title' => (string) ($validated['title'] ?? ''),
                'category' => $this->normalizeCategory($validated['category'] ?? null),
                'notes' => $this->normalizeNullableString($validated['notes'] ?? null),
                'fields' => $fields,
                'status' => $this->normalizeStatus($validated['status'] ?? null, default: PrescriptionModelOptions::STATUS_ACTIVE),
                'created_by' => $userId,
                'updated_by' => $userId,
            ]);
            ...
            DB::rollBack();
            report($exception);
            return response()->json(['message' => 'Não foi possível salvar o modelo de prescrição.'], 422);
        }
    ...
    public function update(Request $request, string $id)
    {
        ...
        try {
            $m->update([
                ...
                'notes' => $this->normalizeNullableString($validated['notes'] ?? null),
                'fields' => $fields,
                'status' => $this->normalizeStatus($validated['status'] ?? null, default: (string) ($m->status ?? PrescriptionModelOptions::STATUS_ACTIVE)),
                'updated_by' => $userId,
            ]);
        } catch (Throwable $exception) {
            report($exception);
            return response()->json(['message' => 'Não foi possível atualizar o modelo de prescrição.'], 422);
        }
    ...
    public function destroy(string $id)
    {
        ...
        try {
            ...
            DB::rollBack();
            report($exception);
            return response()->json(['message' => 'Não foi possível remover o modelo de prescrição.'], 422);
        }
    ...
    private function validatePayload(Request $request): array
    {
        return $request->validate([
            ...
            'status' => ['required', 'string', 'in:' . implode(',', PrescriptionModelOptions::statuses())],
            'fields' => ['array'],
    ...
    private function normalizeCategory(mixed $value): string
    {
        ...
        $map = PrescriptionModelOptions::categories(); // key => label
    ...
    private function normalizeStatus(mixed $value, string $default): string
    {
        ...
        return in_array($raw, PrescriptionModelOptions::statuses(), true) ? $raw : $default;
    }
    ...
    private function normalizeFieldsForStorage(array $fields): array
    {
        ...
        foreach ($fields as $field) {
            ...
            if (! in_array($type, PrescriptionModelOptions::fieldTypes(), true)) continue;

            $config = [];
            foreach (PrescriptionModelOptions::configKeysForType($type) as $key) {
                $val = $field[$key] ?? null;
    ...
    private function toV2Payload(ModeloPrescricao $m): array
    {
        return [
            'id' => (string) $m->id,
            ...
            'status' => (string) (($m->status ?? '') === PrescriptionModelOptions::STATUS_INACTIVE ? PrescriptionModelOptions::STATUS_INACTIVE : PrescriptionModelOptions::STATUS_ACTIVE),
