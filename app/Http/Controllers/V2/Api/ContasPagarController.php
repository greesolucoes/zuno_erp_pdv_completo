...
class ContasPagarController extends Controller
{
    public function index(Request $request)
    {
        ...
        $query = ContaPagar::query()
            ...
            ->when($fornecedorId, fn ($q) => $q->where('fornecedor_id', (int) $fornecedorId))
            ...
            ->when($ordem !== null && $ordem !== '', fn ($q) => $q->orderBy($filtroData, 'asc'))
            ->when($ordem === null || $ordem === '', fn ($q) => $q->orderBy('created_at', 'asc'));

        $data = $query->paginate((int) env('PAGINACAO', 10))->appends($request->all());

        return response()->json([
            'data' => $data->getCollection()->map(fn (ContaPagar $c) => $this->toV2Payload($c))->values(),
            'meta' => [
    ...
    public function options()
    {
        ...
        $fornecedores = Fornecedor::query()
            ...
            ->limit(500)
            ->get(['id', 'razao_social'])
            ->map(fn (Fornecedor $f) => ['id' => (string) $f->id, 'label' => (string) ($f->razao_social ?? '')])
            ->values();
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $valorPago = $this->parseMoneyBr((string) ($validated['valor_pago'] ?? '0'));
            $dataPagamento = (string) ($validated['data_pagamento'] ?? '');

            if ($status === 1) {
                if ($valorPago <= 0) $valorPago = $valorIntegral;
            ...
            $c = ContaPagar::create([
                ...
                'fornecedor_id' => (int) $validated['fornecedor_id'],
            ...
            DB::rollBack();
            report($exception);
            return response()->json(['message' => 'Não foi possível salvar a conta a pagar.'], 422);
        }
    ...
    public function update(Request $request, string $id)
    {
        ...
        try {
            ...
            DB::rollBack();
            report($exception);
            return response()->json(['message' => 'Não foi possível atualizar a conta a pagar.'], 422);
        }
    ...
    public function destroy(string $id)
    {
        ...
        try {
            ...
            report($exception);
            return response()->json(['message' => 'Não foi possível excluir a conta a pagar.'], 422);
        }

        return response()->json(['ok' => true]);
    }

    private function toV2Payload(ContaPagar $c): array
    {
        return [
            'id' => (string) $c->id,
            ...
            'fornecedor_nome' => (string) ($c->fornecedor?->razao_social ?? ''),
            ...
            'valor_pago' => $this->formatMoneyBr((float) ($c->valor_pago ?? 0)),
