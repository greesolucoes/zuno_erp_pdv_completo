...
import { listModelosPrescricao, loadModeloPrescricaoOptions, type ModeloPrescricao, type ModeloPrescricaoLoadOptions } from '../../../../../services/petshop/vet/cadastros/modeloPrescricao.service'
...
const modelos = ref<ModeloPrescricao[]>([])
...
const allRows = computed<Row[]>(() => {
  ...
  return modelos.value.map((m) => ({
    ...
    status: loaded ? findLabel(loaded.status, m.status) : m.status,
  }))
})
...
async function fetchData() {
  loading.value = true
  try {
    const [loadedOptions, list] = await Promise.all([loadModeloPrescricaoOptions(), listModelosPrescricao({ busca: busca.value, page: page.value })])
...
function goToPage(targetPage: number) {
  ...
  router.push({ name: 'petshop-vet-modelo-prescricao', query: { ...route.query, page: safePage === 1 ? undefined : String(safePage) } })
}
...
function onRecarregar() {
  router.push({ name: 'petshop-vet-modelo-prescricao', query: { ...route.query, page: undefined } })
}
...
<template>
  ...
  <h2>Modelo de Prescrição</h2>

  <div id="pnlIdentificacao" class="pnlCollapse">
    <div class="retratil">
      <div class="bd-callout bd-callout-info">Cadastro de modelos de prescrição. Use o campo de pesquisa para localizar por título, categoria ou status.</div>
    </div>
  </div>
  ...
  <div id="pnlComandos" class="navbar navbar-nfse">
    <div class="navbar-collapse" id="searchbar">
      <ul class="nav navbar-nav">
        <li>
          <RouterLink id="btnNovoModeloPrescricao" to="/Petshop/Vet/Cadastros/ModeloPrescricao/Novo" class="btn btn-lg btn-primary">
        ...
        <td class="td-opcoes">
          <div class="menu-suspenso-tabela">
            ...
            <div class="list-group menu-content" style="display: none">
              <RouterLink :to="`/Petshop/Vet/Cadastros/ModeloPrescricao/${(row as any).id}`" class="list-group-item">
                <img src="/img/op-visualizar.svg" />Visualizar
              </RouterLink>
              <RouterLink :to="`/Petshop/Vet/Cadastros/ModeloPrescricao/${(row as any).id}/Editar`" class="list-group-item">
