...
const allRows = computed<ChecklistRow[]>(() => {
  ...
  return checklists.map((c) => ({
    ...
    qtdItens: String((c.itens ?? []).filter((i) => (i?.texto ?? '').trim().length > 0).length),
    atualizadoEm: formatDateTimeBr(c.updated_at),
    descricao: compactText(c.descricao) || '-',
  }))
})
...
function onRecarregar() {
  router.push({ name: 'petshop-vet-checklist', query: { ...route.query, page: undefined } })
}
...
<template>
  ...
  <div id="pnlIdentificacao" class="pnlCollapse">
    <div class="retratil">
      <div class="bd-callout bd-callout-info">Cadastro de checklists. Use o campo de pesquisa para localizar por título, itens ou descrição.</div>
    </div>
  </div>
  ...
  <div id="pnlComandos" class="navbar navbar-nfse">
    <div class="navbar-collapse" id="searchbar">
      <ul class="nav navbar-nav">
        <li>
          <RouterLink id="btnNovoChecklist" to="/Petshop/Vet/Cadastros/Checklist/Novo" class="btn btn-lg btn-primary">
            <img src="/img/btn-novo.svg" /><span>Novo checklist</span>
          </RouterLink>
        </li>
        <li>
          <a
      ...
      <form class="navbar-form" method="get" @submit.prevent="onSubmit">
        <div class="form-group" style="display: inline">
          <div class="input-group input-group-lg" style="display: table">
            <input v-model="busca" class="form-control" name="busca" placeholder="Pesquisar por título, itens ou descrição" type="text" />
  ...
  >
    <template #head>
      <tr>
        ...
        <th style="width: 10%">Qtd. Itens</th>
    ...
    <template #row="{ row }">
      <tr :data-id="(row as any).id">
        ...
        <td class="td-center">{{ (row as any).qtdItens }}</td>
        ...
        <td class="td-opcoes">
          <div class="menu-suspenso-tabela">
            ...
            <div class="list-group menu-content" style="display: none">
              <RouterLink :to="`/Petshop/Vet/Cadastros/Checklist/${(row as any).id}`" class="list-group-item">
                <img src="/img/op-visualizar.svg" />Visualizar
              </RouterLink>
              <RouterLink :to="`/Petshop/Vet/Cadastros/Checklist/${(row as any).id}/Editar`" class="list-group-item">
