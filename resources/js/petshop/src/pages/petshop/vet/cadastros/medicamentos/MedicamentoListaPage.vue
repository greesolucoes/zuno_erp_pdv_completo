...
import { listMedicamentos, loadMedicamentosOptions, type Medicamento, type MedicamentosLoadOptions } from '../../../../../services/petshop/vet/cadastros/medicamentos.service'
...
const allRows = computed<MedicamentoRow[]>(() => {
  ...
  return medicamentos.value.map((m) => {
...
async function fetchData() {
  loading.value = true
  try {
    const [loadedOptions, list] = await Promise.all([loadMedicamentosOptions(), listMedicamentos({ busca: busca.value, page: page.value })])
    options.value = loadedOptions
    medicamentos.value = list.data
