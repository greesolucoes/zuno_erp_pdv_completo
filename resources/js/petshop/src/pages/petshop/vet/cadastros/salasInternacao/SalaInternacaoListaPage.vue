...
import { listSalasInternacao, loadSalasInternacaoOptions, type SalaInternacao, type SalasInternacaoLoadOptions } from '../../../../../services/petshop/vet/cadastros/salasInternacao.service'
...
const salas = ref<SalaInternacao[]>([])
...
async function fetchData() {
  ...
  try {
    const [loadedOptions, list] = await Promise.all([
      ...
      listSalasInternacao({ busca: busca.value, page: page.value }),
    ])
...
function goToPage(targetPage: number) {
  ...
  router.push({ name: 'petshop-vet-salas-internacao', query: { ...route.query, page: safePage === 1 ? undefined : String(safePage) } })
}
...
function onRecarregar() {
  router.push({ name: 'petshop-vet-salas-internacao', query: { ...route.query, page: undefined } })
}
...
<template>
  ...
  <h2>Salas de Internação</h2>

  <div id="pnlIdentificacao" class="pnlCollapse">
    <div class="retratil">
      <div class="bd-callout bd-callout-info">Cadastro de salas de internação. Use o campo de pesquisa para localizar por nome, tipo ou status.</div>
    </div>
  </div>
  ...
  <div id="pnlComandos" class="navbar navbar-nfse">
    <div class="navbar-collapse" id="searchbar">
      <ul class="nav navbar-nav">
        <li>
          <RouterLink id="btnNovaSalaInternacao" to="/Petshop/Vet/Cadastros/SalasInternacao/Novo" class="btn btn-lg btn-primary">
        ...
        <td class="td-opcoes">
          <div class="menu-suspenso-tabela">
            ...
            <div class="list-group menu-content" style="display: none">
              <RouterLink :to="`/Petshop/Vet/Cadastros/SalasInternacao/${(row as any).id}`" class="list-group-item">
                <img src="/img/op-visualizar.svg" />Visualizar
              </RouterLink>
              <RouterLink :to="`/Petshop/Vet/Cadastros/SalasInternacao/${(row as any).id}/Editar`" class="list-group-item">
