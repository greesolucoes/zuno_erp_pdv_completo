...
import { listSalasInternacao, loadSalasInternacaoOptions, type SalasInternacaoLoadOptions } from '../../../../../services/petshop/vet/cadastros/salasInternacao.service'
...
const allRows = computed<SalaRow[]>(() => {
  ...
  const salas = listSalasInternacao(busca.value)
  return salas.map((s) => ({
    id: s.id,
    sala: s.nome,
...
onMounted(() => {
  loadSalasInternacaoOptions().then((o) => {
    options.value = o
  })
