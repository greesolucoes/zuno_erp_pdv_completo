...
import { createCategoriaServicoDraft, type CategoriaServicoDraft, type CategoriaServicoUpsertPayload } from '../../../composables/createCategoriaServicoDraft'
import { getCategoriaServicoById, loadCategoriasServicoOptions, updateCategoriaServico, type CategoriasServicoLoadOptions } from '../../../services/servicos/categoriasServico.service'
...
async function onSave(payload: CategoriaServicoUpsertPayload) {
  await updateCategoriaServico(id.value, payload)
  router.push({ name: 'categorias-servico' })
}
...
<template>
  ...
  <CategoriaServicoForm v-else mode="edit" :model-value="model" :load-options="options" :on-save="onSave" :on-cancel="onCancel" />
</template>
