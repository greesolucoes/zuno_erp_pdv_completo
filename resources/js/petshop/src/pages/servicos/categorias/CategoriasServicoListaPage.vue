...
} from '../../../services/servicos/categoriasServico.service'
...
const resp = ref<PaginatedResponse<CategoriaServico> | null>(null)
...
type Row = { id: string; nome: string; marketplace: string }
...
const rows = computed<Row[]>(() => {
  ...
  return data.map((c) => ({
    id: c.id,
    nome: c.nome,
    marketplace: c.marketplace === '1' ? 'Sim' : 'Não',
  }))
})
...
function goToPage(targetPage: number) {
  ...
  router.push({ name: 'categorias-servico', query: { ...route.query, page: safePage === 1 ? undefined : String(safePage) } })
}

function onSubmit() {
  router.push({ name: 'categorias-servico', query: { ...route.query, busca: busca.value || undefined, page: undefined } })
}

function onRecarregar() {
  router.push({ name: 'categorias-servico', query: { ...route.query, page: undefined } })
}
...
async function fetchData() {
  loading.value = true
  try {
    resp.value = await listCategoriasServico({ busca: busca.value, page: page.value })
  } finally {
    loading.value = false
    await nextTick()
  ...
  <div id="pnlComandos" class="navbar navbar-nfse">
    <div class="navbar-collapse" id="searchbar">
      <ul class="nav navbar-nav">
        <li>
          <RouterLink id="btnNovaCategoriaServico" to="/servicos/categorias/novo" class="btn btn-lg btn-primary">
        ...
        <td class="td-opcoes">
          <div class="menu-suspenso-tabela">
            ...
            <div class="list-group menu-content" style="display: none">
              <RouterLink :to="`/servicos/categorias/${(row as any).id}`" class="list-group-item">
                <img src="/img/op-visualizar.svg" />Visualizar
              </RouterLink>
              <RouterLink :to="`/servicos/categorias/${(row as any).id}/editar`" class="list-group-item">
