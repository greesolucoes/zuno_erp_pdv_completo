...
import { getServicoById, loadServicosOptions, updateServico, type ServicoUpsertPayload, type ServicosLoadOptions } from '../../services/servicos/servicos.service'
...
async function onSave(payload: ServicoUpsertPayload) {
  await updateServico(id.value, payload)
  router.push({ name: 'servicos' })
}
...
<template>
  ...
  <ServicoFormWizard v-else mode="edit" :model-value="model" :load-options="options" :on-save="onSave" :on-cancel="onCancel" />
</template>
