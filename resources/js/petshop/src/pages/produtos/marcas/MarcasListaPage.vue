...
import { deleteMarca, listMarcas, type Marca, type PaginatedResponse } from '../../../services/produtos/marcas.service'
...
const resp = ref<PaginatedResponse<Marca> | null>(null)
...
function goToPage(targetPage: number) {
  ...
  router.push({ name: 'marcas', query: { ...route.query, page: safePage === 1 ? undefined : String(safePage) } })
}

function onSubmit() {
  router.push({ name: 'marcas', query: { ...route.query, busca: busca.value || undefined, page: undefined } })
}

function onRecarregar() {
  router.push({ name: 'marcas', query: { ...route.query, page: undefined } })
}
...
async function fetchData() {
  loading.value = true
  try {
    resp.value = await listMarcas({ busca: busca.value, page: page.value })
  } finally {
    loading.value = false
    await nextTick()
  ...
  <div id="pnlComandos" class="navbar navbar-nfse">
    <div class="navbar-collapse" id="searchbar">
      <ul class="nav navbar-nav">
        <li>
          <RouterLink id="btnNovaMarca" to="/produtos/marcas/novo" class="btn btn-lg btn-primary">
            <img src="/img/btn-novo.svg" /><span>Nova marca</span>
          </RouterLink>
        </li>
        <li>
        ...
        <td class="td-opcoes">
          <div class="menu-suspenso-tabela">
            ...
            <div class="list-group menu-content" style="display: none">
              <RouterLink :to="`/produtos/marcas/${(row as any).id}`" class="list-group-item">
                <img src="/img/op-visualizar.svg" />Visualizar
              </RouterLink>
              <RouterLink :to="`/produtos/marcas/${(row as any).id}/editar`" class="list-group-item">
