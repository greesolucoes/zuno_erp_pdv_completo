...
import {
  ...
  updateUnidadeMedida,
  type UnidadesMedidaLoadOptions,
} from '../../../services/produtos/unidadesMedida.service'
...
async function onSave(payload: UnidadeMedidaUpsertPayload) {
  await updateUnidadeMedida(id.value, payload)
  router.push({ name: 'unidades-medida' })
}
...
<template>
  ...
  <UnidadeMedidaForm v-else mode="edit" :model-value="model" :load-options="options" :on-save="onSave" :on-cancel="onCancel" />
</template>
