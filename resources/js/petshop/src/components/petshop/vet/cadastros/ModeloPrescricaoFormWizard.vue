...
import {
  createModeloPrescricaoDraft,
  type ModeloPrescricaoDraft,
  type ModeloPrescricaoFieldDraft,
  type ModeloPrescricaoFieldType,
  type ModeloPrescricaoUpsertPayload,
} from '../../../../composables/createModeloPrescricaoDraft'
import { initLegacyUiBindings, loadLegacyStyleOnce } from '../../../../utils/legacyScripts'
import type { ModeloPrescricaoLoadOptions } from '../../../../services/petshop/vet/cadastros/modeloPrescricao.service'
...
const props = defineProps<{
  mode: Mode
  modelValue: ModeloPrescricaoDraft
  loadOptions: ModeloPrescricaoLoadOptions
  onSave?: (payload: ModeloPrescricaoUpsertPayload) => void | Promise<void>
  onCancel?: () => void
}>()
...
const { draft, reset, toPayload, emptyField } = createModeloPrescricaoDraft(props.modelValue)
...
const hasValidTab1 = computed(() => {
  ...
  const statusOk = draft.status === 'ativo' || draft.status === 'inativo'
  const customOk = categoryChoice.value !== CUSTOM || customCategory.value.trim().length > 0
  return titleOk && statusOk && customOk
})
...
function applyTemplate(templateId: string) {
  ...
  const next: ModeloPrescricaoFieldDraft[] = []

  if (t === 'prescricao_basica') {
    next.push(
      { ...emptyField, label: 'Medicamento', type: 'texto_curto', placeholder: 'Nome do medicamento' },
      { ...emptyField, label: 'Dose', type: 'texto_curto', placeholder: 'Ex.: 10mg/kg' },
      { ...emptyField, label: 'Frequência', type: 'texto_curto', placeholder: 'Ex.: 12/12h' },
      { ...emptyField, label: 'Duração', type: 'texto_curto', placeholder: 'Ex.: 7 dias' },
      { ...emptyField, label: 'Observações', type: 'texto_longo', textarea_placeholder: 'Detalhes importantes...' },
    )
  } else if (t === 'antibiotico') {
    next.push(
      { ...emptyField, label: 'Antibiótico', type: 'texto_curto', placeholder: 'Ex.: Amoxicilina' },
      { ...emptyField, label: 'Concentração', type: 'texto_curto', placeholder: 'Ex.: 250mg' },
      { ...emptyField, label: 'Dose (mg/kg)', type: 'numero_decimal', number_min: '0', placeholder: 'Ex.: 10' },
      { ...emptyField, label: 'Frequência', type: 'texto_curto', placeholder: 'Ex.: 12/12h' },
      { ...emptyField, label: 'Duração', type: 'texto_curto', placeholder: 'Ex.: 7 dias' },
      { ...emptyField, label: 'Orientações ao tutor', type: 'texto_longo', textarea_placeholder: 'Orientações...' },
    )
  } else if (t === 'pos_operatorio') {
    next.push(
      { ...emptyField, label: 'Analgesia', type: 'texto_longo', textarea_placeholder: 'Medicamento, dose, frequência...' },
      { ...emptyField, label: 'Cuidados com ferida', type: 'texto_longo', textarea_placeholder: 'Curativos, higiene...' },
      { ...emptyField, label: 'Restrição de atividade', type: 'texto_longo', textarea_placeholder: 'Repouso, colar elizabetano...' },
      { ...emptyField, label: 'Retorno', type: 'texto_curto', placeholder: 'Ex.: 7 dias' },
    )
  }
...
function optionsForType(field: ModeloPrescricaoFieldDraft) {
...
const validationIssues = computed(() => {
  const issues: string[] = []
  if (!hasValidTab1.value) issues.push('Preencha Título e Status; se Categoria for personalizada, preencha a categoria (Tab 1).')
...
function previewComponentTag(typeValue: string) {
  const t = typeValue as ModeloPrescricaoFieldType
...
<template>
  ...
  <form class="formdps" action="#" method="post" novalidate @submit.prevent="onPrimary">
          ...
          <div class="row" style="margin-top: 5px; margin-bottom: 0">
            <div class="col-md-12">
              <div class="form-group form-group-lg">
                ...
                <select v-model="categoryChoice" class="form-control" name="category_choice" :disabled="isReadOnly">
          ...
          <div class="row" style="margin-top: 5px; margin-bottom: 0">
            <div class="col-md-8">
              <div class="bd-callout bd-callout-info" style="margin: 0">
                Use um modelo pronto para iniciar rapidamente. Você pode ajustar os campos depois.
              </div>
            </div>
            <div class="col-md-4">
          ...
          <div class="row" style="margin-top: 5px; margin-bottom: 0">
            <div class="col-md-12" style="display: flex; justify-content: space-between; align-items: center; gap: 10px">
              <div class="bd-callout bd-callout-info" style="margin: 0; flex: 1">
                Adicione os campos da prescrição. Cada campo precisa de <strong>Label</strong> e <strong>Tipo</strong>.
              </div>
                  ...
                  <div class="row" style="margin: 0">
                    <div class="col-md-10">
                      <div class="form-group form-group-lg" style="margin: 0">
                        ...
                        <input v-model="field.label" class="form-control" :readonly="isReadOnly" placeholder="Ex.: Medicamento" type="text" />
                      </div>
                    </div>
    ...
    <template v-else>
      <div class="pnlCollapse semi-aberto">
        <h2>Validações de consistência</h2>
        <div class="retratil">
    ...
    <div class="comandos">
      ...
      <button v-if="!isReadOnly" id="btnAvancar" type="submit" class="btn btn-lg btn-primary direita has-spin" :disabled="step === 1 ? !hasValidTab1 : step === 3 ? !!validationIssues.length : false">
        <span>{{ step < 3 ? 'Avançar' : 'Salvar modelo' }}</span><img src="/img/btn-avancar.svg" />
      </button>
    </div>
  </form>
</template>
