...
import {
  createReservaCrecheDraft,
  type ReservaCrecheDraft,
  type ReservaCrecheProduto,
  type ReservaCrecheServicoExtra,
  type ReservaCrecheUpsertPayload,
} from '../../../composables/createReservaCrecheDraft'
import type { ReservasCrecheLoadOptions, ServicoOption } from '../../../services/petshop/creche/reservas.service'
...
const props = defineProps<{
  mode: Mode
  modelValue: ReservaCrecheDraft
  loadOptions: ReservasCrecheLoadOptions
  onSave?: (payload: ReservaCrecheUpsertPayload) => void | Promise<void>
  onCancel?: () => void
}>()
...
const { draft, reset, toPayload } = createReservaCrecheDraft(props.modelValue)
...
watch(
  () => draft.turma_id,
  (value) => {
    const turma = props.loadOptions.turmas.find((t) => t.id === value) ?? null
    draft.id_turma = turma?.id ?? ''
    draft.nome_turma = turma?.label ?? ''
  },
  { immediate: true },
)

const estadoLocked = computed(() => !String(draft.data_saida ?? '').trim())

const saidaUnlocked = ref(false)
const canUnlockSaida = computed(() => props.mode === 'edit' && !isReadOnly.value && !String(draft.data_saida ?? '').trim())

function unlockSaida() {
  if (!canUnlockSaida.value) return
  saidaUnlocked.value = true
}

const saidaEditable = computed(() => !isReadOnly.value && (String(draft.data_saida ?? '').trim().length > 0 || saidaUnlocked.value))
...
function emptyServicoExtra(): ReservaCrecheServicoExtra {
  return {
    servico_id: '',
...
function applyServicoMeta(row: ReservaCrecheServicoExtra) {
...
const totalServicos = computed(() => (draft.servicos_extras ?? []).reduce((sum, s) => sum + parseMoneyBr(s.servico_valor), 0))

function emptyProduto(): ReservaCrecheProduto {
...
function applyProdutoMeta(row: ReservaCrecheProduto) {
...
const hasValidTab2 = computed(() => {
  const entradaOk = String(draft.data_entrada ?? '').trim().length > 0
  const horaOk = String(draft.horario_entrada ?? '').trim().length > 0
  const turmaOk = String(draft.turma_id ?? '').trim().length > 0
  return entradaOk && horaOk && turmaOk
})
...
<template>
      ...
      <ul class="nav nav-tabs" role="tablist">
        ...
        <li :class="step === 3 ? 'ativo' : 'fechado'">
          <a href="javascript:void(0);" @click.prevent="goToStep(3)">
            <span class="round-tab">
              <img class="passos-valores-fechado-ico" src="/img/passos-valores-fechado-ico.svg" />
            </span>
          </a>
  ...
  <form class="formdps" action="#" method="post" novalidate @submit.prevent="onPrimary">
    ...
    <input type="hidden" name="nome_turma" :value="draft.nome_turma" />
    <input type="hidden" name="id_turma" :value="draft.id_turma" />
            ...
            <div class="col-md-12">
              <div class="form-group form-group-lg">
                ...
                <div v-if="estadoLocked" class="bd-callout bd-callout-info" style="margin: 10px 0 0">
                  Situação fica disponível após a saída ser registrada.
                </div>
              </div>
            </div>
          </div>
      ...
      <div class="pnlCollapse semi-aberto">
        ...
        <div style="padding: 0 20px">
          <div class="row" style="margin-top: 5px; margin-bottom: 0">
            <div class="col-md-4">
              <div class="form-group">
                <label class="control-label"><span>Data de entrada<span class="asterisco">*</span></span></label>
                <div class="input-group input-group-lg">
                  <input v-model="draft.data_entrada" class="form-control data" name="data_entrada" :readonly="isReadOnly" required type="text" />
            ...
            <div class="col-md-4">
              <div class="form-group form-group-lg">
                <label class="control-label"><span>Horário de entrada<span class="asterisco">*</span></span></label>
                <input v-model="draft.horario_entrada" class="form-control" name="horario_entrada" :readonly="isReadOnly" required type="time" />
              </div>
            </div>
          </div>
          ...
          <div class="row" style="margin-top: 5px; margin-bottom: 0">
            <div class="col-md-4">
              <div class="form-group">
                <label class="control-label"><span>Data de saída</span></label>
                <div class="input-group input-group-lg">
                  <input v-model="draft.data_saida" class="form-control data" name="data_saida" :readonly="!saidaEditable" :disabled="!saidaEditable" type="text" />
                  <span class="input-group-btn">
                    <button class="btn btn-lg btn-default btnCalendario" data-original-title="Abrir calendário" data-toggle="tooltip" type="button" :disabled="!saidaEditable">
                      <div class="btn-calendario"></div>
                    </button>
                  </span>
                </div>
            ...
            <div class="col-md-4">
              <div class="form-group form-group-lg">
                <label class="control-label"><span>Horário de saída</span></label>
                <input v-model="draft.horario_saida" class="form-control" name="horario_saida" :readonly="!saidaEditable" :disabled="!saidaEditable" type="time" />
              </div>
            </div>
            <div class="col-md-4" style="text-align: right; padding-top: 28px">
              <button v-if="canUnlockSaida" type="button" class="btn btn-lg btn-info" @click.prevent="unlockSaida">Registrar saída</button>
            </div>
          </div>
          ...
          <div class="row" style="margin-top: 5px; margin-bottom: 0">
            <div class="col-md-12">
              <div class="form-group form-group-lg">
                <label class="control-label"><span>Turma<span class="asterisco">*</span></span></label>
                <select v-model="draft.turma_id" class="form-control form-select2" name="turma_id" :disabled="isReadOnly" required>
                  <option value=""></option>
                  <option v-for="t in props.loadOptions.turmas" :key="t.id" :value="t.id">{{ t.label }} — {{ t.unidade }}</option>
                </select>
              </div>
            </div>
          </div>
    ...
    <div class="comandos">
      ...
      <button
        ...
        :disabled="step === 1 ? !hasValidTab1 : step === 2 ? !hasValidTab2 : !hasValidTab1 || !hasValidTab2"
      >
