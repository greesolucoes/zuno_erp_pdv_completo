...
import type { ContaPagarUpsertPayload, ContasPagarOptions } from '../../services/financeiro/contasPagar.service'
...
const props = defineProps<{
  mode: Mode
  modelValue: ContaPagarDraft
  loadOptions: ContasPagarOptions
  onSave?: (payload: ContaPagarUpsertPayload) => void | Promise<void>
  onCancel?: () => void
}>()
...
const { draft, reset, toPayload } = createContaPagarDraft(props.modelValue)
...
const canAdvanceFrom1 = computed(() => {
  return String(draft.fornecedor_id ?? '').trim().length > 0 && String(draft.data_vencimento ?? '').trim().length > 0
})
...
watch(
  ...
  (value) => {
    if (value !== '1') {
      draft.data_pagamento = ''
      draft.valor_pago = '0,00'
    } else {
      if (!String(draft.data_pagamento ?? '').trim()) {
        const today = new Date()
        const yyyy = String(today.getFullYear())
        const mm = String(today.getMonth() + 1).padStart(2, '0')
        const dd = String(today.getDate()).padStart(2, '0')
        draft.data_pagamento = `${yyyy}-${mm}-${dd}`
      }
      if (!String(draft.valor_pago ?? '').trim() || String(draft.valor_pago) === '0,00') {
        draft.valor_pago = String(draft.valor_integral ?? '0,00')
      }
    }
  },
)
...
watch(
  () => [
    ...
    draft.local_id,
    draft.fornecedor_id,
    draft.categoria_conta_id,
    draft.tipo_pagamento,
    draft.status,
  ],
  async () => {
    await nextTick()
...
<template>
    ...
    <div class="wizard-inner">
      ...
      <ul class="nav nav-tabs" role="tablist">
        ...
        <li :class="step === 2 ? 'ativo' : 'fechado'">
          ...
          <a href="javascript:void(0);" class="tag" @click.prevent="goToStep(2)"><span class="tag-text">Financeiro</span></a>
        </li>
      </ul>
    </div>
  </div>
  ...
  <form class="formdps" action="#" method="post" novalidate @submit.prevent="onPrimary">
      ...
      <div class="pnlCollapse semi-aberto">
        ...
        <div style="padding: 0 20px">
          <div class="row" style="margin-top: 5px; margin-bottom: 0">
            ...
            <div class="col-md-5">
              <div class="form-group form-group-lg">
                <label class="control-label"><span>Fornecedor<span class="asterisco">*</span></span></label>
                <select v-model="draft.fornecedor_id" class="form-control form-select2" name="fornecedor_id" :disabled="isReadOnly" required>
                  <option value="">Selecione</option>
                  <option v-for="f in props.loadOptions.fornecedores" :key="f.id" :value="f.id">{{ f.label }}</option>
                </select>
              </div>
            </div>
          </div>
      ...
      <div class="pnlCollapse semi-aberto">
        ...
        <div style="padding: 0 20px">
            ...
            <div class="col-md-3">
              <div class="form-group form-group-lg">
                <label class="control-label">
                  ...
                  <a
                    data-content="Marque Sim se esta conta já foi paga. Se marcar Não, ela ficará como pendente."
                    data-placement="top"
                    data-toggle="popover"
                    data-trigger="focus"
                    role="button"
          ...
          <div v-if="draft.status === '1'" class="row" style="margin-top: 5px; margin-bottom: 0">
            <div class="col-md-3">
              <div class="form-group form-group-lg">
                <label class="control-label"><span>Data pagamento</span></label>
                <input v-model="draft.data_pagamento" class="form-control" name="data_pagamento" :readonly="isReadOnly" type="date" />
              </div>
            </div>
            <div class="col-md-3">
              <div class="form-group form-group-lg">
                <label class="control-label"><span>Valor pago</span></label>
                <input v-model="draft.valor_pago" class="form-control moeda" name="valor_pago" :readonly="isReadOnly" type="text" />
              </div>
            </div>
          </div>
        </div>
    ...
    <div class="comandos">
      ...
      <button v-if="!isReadOnly && step === 2" id="btnSalvarContaPagar" type="submit" class="btn btn-lg btn-primary direita has-spin" :disabled="saving">
