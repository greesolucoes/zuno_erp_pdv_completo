function AplicarMascaras() {
    ...
    AplicarMascara(".cpf", "999.999.999-99");
    AplicarMascara(".cnpj", "99.999.999/9999-99");
    AplicarMascara(".telefone", ["(99)9999-9999", "(99)99999-9999"]);
    AplicarMascara(".cep", "99.999-999");
    AplicarMascara(".mes-ano", "99/9999");
    AplicarMascara(".data", "99/99/9999");
    AplicarMascara(".numerico", "999999999999");
    AplicarMascara(".num7pos", "9999999");
    AplicarMascara(".num14pos", "99999999999999");
    AplicarMascara(".num15pos", "999999999999999");
    AplicarMascara(".num30pos", "999999999999999999999999999999");
    AplicarMascara(".string60pos", "SSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSS");
    AplicarMascara(".reciboIRPF", "99.99.99.99.99");
    AplicarMascara(".chaveNFe", "99999999999999999999999999999999999999999999");
    AplicarMascara(".chaveNFSe", "99999999999999999999999999999999999999999999999999")
}
function AplicarMascara(n, t, i) {
    if (typeof n == "undefined" || n == "" || typeof t == "undefined" || t == "")
        return !1;
    var r = t;
    if ($.isArray(t) && t.length > 2)
        return !1;
    $(n).each(function() {
        if ($.isArray(t) && (r = NormalizaValor($(this).val()).length <= NormalizaValor(t[0]).length ? t[0] : t[1]),
        $(this).is("input, textarea")) {
            if (VMasker($(this)[0]).unMask(),
            VMasker($(this)[0]).maskPattern(r),
            $(this).off("blur.limparValorCampoMascarado"),
            typeof i != "undefined" && i)
                $(this).on("blur.limparValorCampoMascarado", function() {
                    $(this).val().length != r.length && $(this).val("")
                });
            $(this).off("blur.acionarKeyupNaPerdaDeFoco");
            $(this).on("blur.acionarKeyupNaPerdaDeFoco", function() {
                $(this).keyup()
            });
            if ($.isArray(t))
                $(this).on("keyup", function() {
                    var i = $(this).val().replace(/\D/g, "")
                      , n = 0;
                    NormalizaValor($(this).val()).length > NormalizaValor(t[n]).length && (n = 1);
                    VMasker($(this)).unMask();
                    VMasker($(this)).maskPattern(t[n]);
                    $(this).val(VMasker.toPattern(i, t[n]))
                })
        } else
            $(this).text(VMasker.toPattern($(this).text(), r))
    })
}
function NormalizaValor(n) {
    var t = n.replace(/\-/g, "");
...
function FuncoesBasicas() {
    ...
    $(".data").siblings(".input-group-btn").find("button").on("click", function(n) {
        n.preventDefault();
        $(this).closest(".input-group").find(".data").datepicker("show")
    });
    $("form .checkbox input[type=hidden]").each(function() {
        $(this).remove()
    });
    $(".form-chosen").chosen({
        width: "100%",
    ...
    $("form .field-validation-error").each(function() {
        $(this).closest(".form-group").addClass("erro")
    });
    $(".form-group.erro input[type=radio], .form-group.erro select, .form-group.erro input[type=text], .form-group.erro textarea").change(function() {
        $(this).closest(".form-group").removeClass("erro");
        $(this).closest(".form-group").find(".field-validation-error").hide()
    });
    $("form").on("keydown", ".monetario, .percentual", function(n) {
        if ($.inArray(n.keyCode, [46, 8, 9, 27, 13, 110, 190]) === -1 && (n.keyCode !== 65 || n.ctrlKey !== !0 && n.metaKey !== !0) && (!(n.keyCode >= 35) || !(n.keyCode <= 40)) && (n.shiftKey || n.keyCode < 48 || n.keyCode > 57) && (n.keyCode < 96 || n.keyCode > 105)) {
            n.preventDefault();
            return
        }
    });
    $("form").on("keydown", ".form-control", function(n) {
        if (n.key == "<" || n.key == ">") {
            n.preventDefault();
            return
        }
    });
    $("form").on("keyup", ".monetario", function() {
        var n = $(this).val()
          , t = $(this).hasClass("aceita-zero");
        $(this).val(FormataValorMonetario(n, t))
    });
    $("form").on("keyup", ".percentual", function() {
        var n = $(this).val()
          , t = $(this).hasClass("aceita-zero")
          , i = $(this).hasClass("maximo999") ? 999 : 1e4;
        $(this).val(FormataValorPercentual(n, t, i))
    });
    $("form").on("change", ".monetario, .percentual", function() {
        var n = $(this).val(), i = $(this).hasClass("aceita-zero"), t;
        n.indexOf(",") == -1 ? n += ",00" : (t = n.split(",").pop(),
        t.length == 1 && (n += "0"));
        $(this).val(FormataValorMonetario(n, i))
    });
    $("form .monetario, form .percentual").each(function() {
        var n = $(this).val()
          , t = $(this).hasClass("aceita-zero");
        $(this).val(FormataValorMonetario(n, t))
    });
...
function BuscarCEP(n, t) {
    $.ajax({
        ...
        success: function(n) {
            t.find('input[id$="_CodigoMunicipio"]').val(n.CodigoCompletoMunicipio);
            t.find('input[id$="_NomeMunicipio"]').val(n.Municipio.toUpperCase() + "/" + n.SiglaUF.toUpperCase());
            t.find('input[id$="_Bairro"]').val(n.Bairro);
            t.find('input[id$="_Logradouro"]').val(n.TipoLogradouro + " " + n.Logradouro)
        },
        error: function(n) {
            LimparEndereco(t);
            n.status == 429 ? ExibirAlerta("Você estourou o limite de consultas ao CEP. Tente novamente mais tarde.") : n.status == 404 ? ExibirAlerta("CEP não encontrado.") : ExibirAlerta("Não foi possível obter os dados do CEP consultado")
        }
    })
}
function ReiniciarPessoa(n) {
    n.find('div[id$="EnderecoBrasil"]').show();
    n.find('div[id$="EnderecoExterior"]').hide();
    n.find('input[id$="InformarEndereco"]').prop("checked", !1);
    n.find('div[id$="Endereco"]').slideUp();
    LimparPessoa(n)
}
function LimparPessoa(n) {
    n.find('input[id$="Inscricao"]').val("");
    n.find('input[id$="InscricaoExterior"]').val("");
    n.find('input[id$="InscricaoMunicipal"]').val("");
    n.find('input[id$="NIF"]').val("");
    n.find('input[id$="Nome"]').val("");
    n.find('input[id$="Telefone"]').val("");
    n.find('input[id$="Email"]').val("");
    AlterarSelecaoChosen(n.find('select[id$="MotivoNaoInformacaoNIF"]'), "");
    n.find('input[id$="NIFInformado"]').prop("checked", !1);
    LimparEndereco(n)
}
function LimparEndereco(n) {
    n.find('input[id$="CEP"]').val("");
    n.find('input[id$="CodigoMunicipio"]').val("");
    n.find('input[id$="NomeMunicipio"]').val("");
    n.find('input[id$="Bairro"]').val("");
    n.find('input[id$="Logradouro"]').val("");
    n.find('input[id$="Numero"]').val("");
    n.find('input[id$="Complemento"]').val("");
    n.find('input[id$="EnderecoExterior_Logradouro"]').val("");
    n.find('input[id$="EnderecoExterior_Numero"]').val("");
    n.find('input[id$="EnderecoExterior_Complemento"]').val("");
    n.find('input[id$="EnderecoExterior_Bairro"]').val("");
    n.find('input[id$="EnderecoExterior_Cidade"]').val("");
    n.find('input[id$="EnderecoExterior_CodigoEnderecamentoPostal"]').val("");
    n.find('input[id$="EnderecoExterior_EstadoProvinciaRegiao"]').val("");
    AlterarSelecaoChosen(n.find('select[id$="EnderecoExterior_CodigoPais"]'), "");
    n.find('input[id$="InformarEndereco"]').prop("checked", !1);
    n.find('div[id$="Endereco"]').slideUp()
}
function FormataValorMonetario(n, t=false) {
    if (n == "" || n == "0,0" || n == ",00")
        return "";
    var i = "";
    return n = NormalizaValor(n),
    n = RemoveZerosAEsquerda(n, t),
    tam = n.length,
    i = tam <= 1 && (n == "" || n == "0") ? "" : "0,0" + n,
    tam == 2 && (i = "0," + n),
    tam > 2 && tam <= 5 && (i = n.substr(0, tam - 2) + "," + n.substr(tam - 2, tam)),
    tam >= 6 && tam <= 8 && (i = n.substr(0, tam - 5) + "." + n.substr(tam - 5, 3) + "," + n.substr(tam - 2, tam)),
    tam >= 9 && tam <= 11 && (i = n.substr(0, tam - 8) + "." + n.substr(tam - 8, 3) + "." + n.substr(tam - 5, 3) + "," + n.substr(tam - 2, tam)),
    tam >= 12 && tam <= 14 && (i = n.substr(0, tam - 11) + "." + n.substr(tam - 11, 3) + "." + n.substr(tam - 8, 3) + "." + n.substr(tam - 5, 3) + "," + n.substr(tam - 2, tam)),
    tam >= 15 && tam <= 18 && (i = n.substr(0, tam - 14) + "." + n.substr(tam - 14, 3) + "." + n.substr(tam - 11, 3) + "." + n.substr(tam - 8, 3) + "." + n.substr(tam - 5, 3) + "," + n.substr(tam - 2, tam)),
    i
}
function FormataValorPercentual(n, t=false, i=1e4) {
    if (n == "" || n == "0,0" || n == ",00")
        return "";
    var r = "";
    return n = NormalizaValor(n),
    n = RemoveZerosAEsquerda(n, t),
    parseFloat(n) > i && (n = Math.floor(n / 10).toString()),
    tam = n.length,
    r = tam <= 1 && n == "" ? "" : "0,0" + n,
    tam == 2 && (r = "0," + n),
    tam > 2 && (r = n.substr(0, tam - 2) + "," + n.substr(tam - 2, tam)),
    r
}
function ArredondarValorToEven(n) {
    const i = Number((n * 100).toFixed(8))
      , t = Math.floor(i)
      , r = i - t;
    let u;
    return u = r > .5 ? Math.ceil(i) : r < .5 ? t : t % 2 == 0 ? t : t + 1,
    Number((u / 100).toFixed(2))
}
function NormalizaValor(n) {
    var t = n.replace(/\-/g, "");
    return t = t.replace(/\./g, ""),
...
function RemoveZerosAEsquerda(n, t=false) {
    var i = n.replace(/^0+/, "");
    return t && n == 0 ? 0 : i
}
...
function AlterarSelecaoChosen(n, t) {
    $(n).find("option").removeAttr("selected");
    $(n).val(t).attr("selected", "selected").trigger("chosen:updated")
}
function BloquearCampo(n) {
    $(n).hasClass("form-chosen") ? $(n).attr("readonly", !0).trigger("chosen:updated") : $(n).attr("readonly", !0);
    $(n).closest(".form-group").removeClass("erro").find(".field-validation-error").empty();
    $(n).parent().find(".input-group-btn button").attr("disabled", "disabled").val("");
    $(n).parent().find(".input-group-addon").addClass("blocked")
}
function DesbloquearCampo(n) {
    $(n).hasClass("form-chosen") ? $(n).attr("readonly", !1).trigger("chosen:updated") : $(n).attr("readonly", !1);
    $(n).parent().find(".input-group-btn button").removeAttr("disabled");
    $(n).parent().find(".input-group-addon").removeClass("blocked")
}
function LimparTodosOsInputs(n) {
    $(n).find("input:text").val("")
}
function RecuperarInfoInscricao(n, t) {
    return function() {
        var i = null;
        return $.ajax({
            url: window.UrlBase + "api/EmissaoDPS/RecuperarInfoInscricao/" + n + "?data=" + t,
            type: "GET",
            dataType: "json",
            async: !1,
            global: !1,
...
function RecuperarInfoPessoaJuridicaTomador(n, t) {
    return function() {
        var i = null;
        return $.ajax({
            url: window.UrlBase + "api/EmissaoDPS/RecuperarInfoPessoaJuridicaTomador/" + n + "?data=" + t,
            type: "GET",
            dataType: "json",
            async: !1,
            global: !1,
...
if (function(n, t) {
    n.fn.confirm = function(t, i) {
        ...
        n(this).each(function() {
            ...
            i.on("click", function(r) {
                ...
                u.$target = i;
                i.attr("href") && Object.keys(u.buttons).length == 0 && (f = n.extend(!0, {}, jconfirm.pluginDefaults.defaultButtons, (jconfirm.defaults || {}).defaultButtons || {}),
                e = Object.keys(f)[0],
                u.buttons = f,
                u.buttons[e].action = function() {
    ...
    n.confirm = function(t, i) {
        ...
        typeof t.buttons != "object" && (t.buttons = {}),
        Object.keys(t.buttons).length == 0) {
    ...
    n.dialog = function(n, t) {
        ...
        n.buttons = {},
        typeof n.closeIcon == "undefined" && (n.closeIcon = function() {}
        ),
        n.confirmKeys = [13],
        jconfirm(n)
    }
    ...
    jconfirm = function(t) {
        var i, r;
        return typeof t == "undefined" && (t = {}),
        i = n.extend(!0, {}, jconfirm.pluginDefaults),
        jconfirm.defaults && (i = n.extend(!0, i, jconfirm.defaults)),
        i = n.extend(!0, {}, i, t),
        r = new Jconfirm(i),
        jconfirm.instances.push(r),
        r
    }
    ;
    Jconfirm = function(t) {
    ...
    Jconfirm.prototype = {
        _init: function() {
            ...
            this.contentParsed = n(document.createElement("div"));
        ...
        _buildHTML: function() {
            ...
            this._watchContent();
            this.animation === "none" && (this.animationSpeed = 1,
        ...
        _parseAnimation: function(t, i) {
            ...
            u = r.join(" ").toLowerCase(),
            i === "o" ? this.animationParsed = u : this.closeAnimationParsed = u,
            u
        },
        ...
        _updateContentMaxHeight: function() {
            var i = n(t).height() - (this.$jconfirmBox.outerHeight() - this.$contentPane.outerHeight()) - (this.offsetTop + this.offsetBottom);
            this.$contentPane.css({
                "max-height": i + "px"
            })
        },
        setBoxWidth: function(n) {
            if (this.useBootstrap) {
                console.warn("cannot set boxWidth, useBootstrap is set to true");
                return
            }
            this.boxWidth = n;
            this.$jconfirmBox.css("width", n)
        },
        ...
        initDraggable: function() {
            ...
            this.draggable) {
                ...
                n(t).on("mousemove." + this._id, function(n) {
                    i.isDrag && (i.movingX = n.clientX - i.mouseX + i.initialX,
                    i.movingY = n.clientY - i.mouseY + i.initialY,
                    i.setDrag())
                });
                n(t).on("mouseup." + this._id, function() {
                    r.removeClass("jconfirm-hand");
                    i.isDrag && (i.isDrag = !1,
        ...
        setDrag: function() {
            ...
            if (this.draggable) {
                ...
                var e = this.$jconfirmBox.outerWidth()
                  , o = this.$jconfirmBox.outerHeight()
                  , s = n(t).width()
                  , h = n(t).height()
                  , i = this
                  , f = 1;
                (i.movingX % f == 0 || i.movingY % f == 0) && (i.dragWindowBorder && (r = s / 2 - e / 2,
                u = h / 2 - o / 2,
                u -= i.dragWindowGap,
                r -= i.dragWindowGap,
                r + i.movingX < 0 ? i.movingX = -r : r - i.movingX < 0 && (i.movingX = r),
                u + i.movingY < 0 ? i.movingY = -u : u - i.movingY < 0 && (i.movingY = u)),
                i.$jconfirmBoxContainer.css("transform", "translate(" + i.movingX + "px, " + i.movingY + "px)"))
            }
        },
        _scrollTop: function() {
        ...
        _watchContent: function() {
            ...
            this._timer = setInterval(function() {
                var u, f, e;
                i.smoothContent && (u = i.$content.outerHeight() || 0,
                u !== r && (i.$contentPane.css({
                    height: u
                }).scrollTop(0),
                r = u),
                f = n(t).height(),
                e = i.offsetTop + i.offsetBottom + i.$jconfirmBox.height() - i.$contentPane.height() + i.$content.height(),
                e < f ? i.$contentPane.addClass("no-scroll") : i.$contentPane.removeClass("no-scroll"))
            }, this.watchInterval)
        },
        ...
        _bindEvents: function() {
            ...
            this.$scrollPane.click(function() {
                ...
                i.boxClicked || (r = !1,
                t = !1,
                n = typeof i.backgroundDismiss == "function" ? i.backgroundDismiss() : i.backgroundDismiss,
                typeof n == "string" && typeof i.buttons[n] != "undefined" ? (r = n,
                t = !1) : t = typeof n == "undefined" || !!n == !0 ? !0 : !1,
                r && (u = i.buttons[r].action.apply(i),
                t = typeof u == "undefined" || !!u),
                t ? i.close() : i.hiLightModal());
                i.boxClicked = !1
            });
        ...
        _setButtons: function() {
            ...
            n.each(this.buttons, function(r, u) {
                ...
                var f = n('<button type="button" class="btn"><\/button>').html(t.buttons[r].text).addClass(t.buttons[r].btnClass).prop("disabled", t.buttons[r].isDisabled).css("display", t.buttons[r].isHidden ? "none" : "").click(function(n) {
                    n.preventDefault();
                    var i = t.buttons[r].action.apply(t, [t.buttons[r]]);
                    t.onAction.apply(t, [r, t.buttons[r]]);
                    t._stopCountDown();
            ...
            this.closeIcon ? (this.closeIconClass && (r = '<i class="' + this.closeIconClass + '"><\/i>',
            this.$closeIcon.html(r)),
            this.$closeIcon.click(function(n) {
                var u, r, i, f;
                n.preventDefault();
                u = !1;
                r = !1;
                i = typeof t.closeIcon == "function" ? t.closeIcon() : t.closeIcon;
                typeof i == "string" && typeof t.buttons[i] != "undefined" ? (u = i,
                r = !1) : r = typeof i == "undefined" || !!i == !0 ? !0 : !1;
                u && (f = t.buttons[u].action.apply(t),
                r = typeof f == "undefined" || !!f);
                r && t.close()
            }),
        ...
        setContentPrepend: function(n) {
            n && this.contentParsed.prepend(n)
        },
        setContentAppend: function(n) {
            n && this.contentParsed.append(n)
        },
        setContent: function(n, t) {
            t = !!t;
            var i = this;
            (n && this.contentParsed.html("").append(n),
            !this.isAjaxLoading || t) && (this.$content.html(""),
        ...
        _parseContent: function() {
            var t = this, r = "&nbsp;", i, u;
            typeof this.content == "function" && (i = this.content.apply(this),
            typeof i == "string" ? this.content = i : typeof i == "object" && typeof i.always == "function" ? (this.isAjax = !0,
            this.isAjaxLoading = !0,
            i.always(function(n, i, r) {
                t.ajaxResponse = {
                    data: n,
                    status: i,
                    xhr: r
            ...
            typeof this.content == "string" && this.content.substr(0, 4).toLowerCase() === "url:" && (this.isAjax = !0,
            this.isAjaxLoading = !0,
            u = this.content.substring(4, this.content.length),
            n.get(u).done(function(n) {
                t.contentParsed.html(n)
        ...
        _startCountDown: function() {
            var r = this, u = this.autoClose.split("|"), t, f, i;
            if (u.length !== 2)
                return console.error("Invalid option for autoClose. example 'close|10000'"),
                !1;
            if (t = u[0],
            f = parseInt(u[1]),
            typeof this.buttons[t] == "undefined")
                return console.error("Invalid button key '" + t + "' for autoClose"),
                !1;
            i = Math.ceil(f / 1e3);
            this.$cd = n('<span class="countdown"> (' + i + ")<\/span>").appendTo(this["$_" + t]);
            this.autoCloseInterval = setInterval(function() {
                r.$cd.html(" (" + (i -= 1) + ") ");
                i <= 0 && (r["$$" + t].trigger("click"),
                r._stopCountDown())
            }, 1e3)
        },
        _getKey: function(n) {
        ...
        reactOnKey: function(t) {
            var e = this, f = n(".jconfirm"), r, u, i;
            if (f.eq(f.length - 1)[0] !== this.$el[0] || (r = t.which,
            this.$content.find(":input").is(":focus") && /13|32/.test(r)))
                return !1;
            u = this._getKey(r);
            u === "esc" && this.escapeKey && (this.escapeKey === !0 ? this.$scrollPane.trigger("click") : (typeof this.escapeKey == "string" || typeof this.escapeKey == "function") && (i = typeof this.escapeKey == "function" ? this.escapeKey() : this.escapeKey,
            i && (typeof this.buttons[i] == "undefined" ? console.warn("Invalid escapeKey, no buttons found with key " + i) : this["$_" + i].trigger("click"))));
        ...
        close: function() {
            var i = this;
            return typeof this.onClose == "function" && this.onClose(),
            this._unwatchContent(),
            n(t).unbind("resize." + this._id),
            n(t).unbind("keyup." + this._id),
            n(t).unbind("jcKeyDown." + this._id),
            this.draggable && (n(t).unbind("mousemove." + this._id),
            n(t).unbind("mouseup." + this._id),
            this.$titleContainer.unbind("mousedown")),
            i.$el.removeClass(i.loadedClass),
            n("body").removeClass("jconfirm-no-scroll-" + i._id),
            ...
            setTimeout(function() {
                ...
                var r = i.closeAnimation === "none" ? 1 : i.animationSpeed;
                setTimeout(function() {
                    var h, r, u, s;
                    for (i.$el.remove(),
                    h = jconfirm.instances,
                    r = jconfirm.instances.length - 1,
                    r; r >= 0; r--)
                        jconfirm.instances[r]._id === i._id && jconfirm.instances.splice(r, 1);
                    if (!jconfirm.instances.length && i.scrollToPreviousElement && jconfirm.lastFocused && jconfirm.lastFocused.length && n.contains(document, jconfirm.lastFocused[0])) {
                        if (u = jconfirm.lastFocused,
                        i.scrollToPreviousElementAnimate) {
                            var e = n(t).scrollTop()
                              , f = jconfirm.lastFocused.offset().top
                              , o = n(t).height();
                            f > e && f < e + o ? u.focus() : (s = f - Math.round(o / 3),
                            n("html, body").animate({
                                scrollTop: s
        ...
        open: function() {
            return this.isOpen() ? !1 : (this._buildHTML(),
            this._bindEvents(),
            this._open(),
            !0)
        },
        setStartingPoint: function() {
            var i = !1, r;
            if (this.animateFromElement !== !0 && this.animateFromElement)
                i = this.animateFromElement,
                jconfirm.lastClicked = !1;
            else if (jconfirm.lastClicked && this.animateFromElement === !0)
                i = jconfirm.lastClicked,
                jconfirm.lastClicked = !1;
            else
                return !1;
            if (!i)
            ...
            var u = f.left + o
              , s = n(t).height() / 2
              , h = n(t).width() / 2
              , c = s - this.$jconfirmBox.outerHeight() / 2
              , l = h - this.$jconfirmBox.outerWidth() / 2;
            if (r -= c,
            u -= l,
            Math.abs(r) > s || Math.abs(u) > h)
                return !1;
            this.$jconfirmBoxContainer.css("transform", "translate(" + u + "px, " + r + "px)")
        },
        _open: function() {
            var n = this;
            ...
            this.$body.focus();
            n.$jconfirmBoxContainer.css("transform", "translate(0px, 0px)");
            setTimeout(function() {
                n.$body.css(n._getCSS(n.animationSpeed, 1));
                n.$body.css({
                    "transition-property": n.$body.css("transition-property") + ", margin"
                });
                n.$jconfirmBoxContainer.addClass("jconfirm-no-transition");
    ...
    jconfirm.lastClicked = !1;
    n(document).on("mousedown", "button, a", function() {
        jconfirm.lastClicked = n(this)
    })
}(jQuery, window),
...
!function(n) {
    ...
    var a = function() {
        ...
        this._check_template(this.o.templates.leftArrow) && this.picker.find(".prev").html('<i class="fa fa-chevron-left"><\/i>');
        this._check_template(this.o.templates.rightArrow) && this.picker.find(".next").html('<i class="fa fa-chevron-right"><\/i>');
        this._buildEvents();
        this._attachEvents();
    ...
    u.prototype = {
        ...
        _process_options: function(t) {
            ...
            e = String(u.orientation).toLowerCase().split(/\s+/g),
        ...
        click: function(t) {
            ...
            this.viewMode === this.o.minViewMode ? this._setDate(f(u, e, s)) : (this.setViewMode(this.viewMode - 1),
            this.fill()));
        ...
        this.each(function() {
            ...
            if (!t) {
                var l = w(this, "date")
                  ...
                  , v = b(a.language)
                  , e = n.extend({}, c, v, l, s);
                o.hasClass("input-daterange") || e.inputs ? (n.extend(e, {
                    inputs: e.inputs || o.find("input").toArray()
                }),
                t = new h(this,e)) : t = new u(this,e);
                o.data("datepicker", t)
            }
            "string" == typeof i && "function" == typeof t[i] && (r = t[i].apply(t, f))
        }),
        r === t || r instanceof u || r instanceof h)
            return this;
        if (this.length > 1)
    ...
    i = {
            ...
            l.length === v.length) {
                ...
                ft = v.length; c < ft; c++) {
                    ...
                    isNaN(p))
                        switch (y) {
                        case "MM":
                            b = n(r[s].months).filter(tt);
                            p = n.inArray(b[0], r[s].months) + 1;
                            break;
                        case "M":
                            b = n(r[s].monthsShort).filter(tt);
        ...
        headTemplate: '<thead><tr><th colspan="7" class="datepicker-title"><\/th><\/tr><tr><th class="prev"><i class="fa fa-chevron-left"><\/i><\/th><th colspan="5" class="datepicker-switch"><\/th><th class="next"><i class="fa fa-chevron-right"><\/i><\/th><\/tr><\/thead>',
...
function(n, t) {
    ...
    n.fn.dropdownHover = function(r) {
        return f = f.add(this.parent()),
        this.each(function() {
            var o = n(this), u = o.parent(), h = {
            ...
            u.hover(function(n) {
                if (!u.hasClass("open") && !o.is(n.target))
                    return !0;
                i && (e.instantlyCloseOthers === !0 && f.removeClass("open"),
                t.clearTimeout(s),
                u.addClass("open"))
            }, function() {
                i && (s = t.setTimeout(function() {
                    u.removeClass("open")
                }, e.delay))
            });
            o.hover(function() {
            ...
            u.find(".dropdown-submenu").each(function() {
                ...
                r.hover(function() {
                    i && (t.clearTimeout(u),
                    r.children(".dropdown-menu").show(),
                    r.siblings().children(".dropdown-menu").hide())
                }, function() {
                    var n = r.children(".dropdown-menu");
                    i ? u = t.setTimeout(function() {
                        n.hide()
                    }, e.delay) : n.hide()
                })
            })
    ...
    n(document).ready(function() {
        ...
        n(document).mousemove(function(t) {
            r.hits++;
            r.hits > 20 || Math.abs(t.pageX - r.x) + Math.abs(t.pageY - r.y) < 4 ? (n(this).unbind(t),
            i = !0) : (r.x = t.pageX,
            r.y = t.pageY)
        })
    });
    e = ".dropdown-submenu:hover>.dropdown-menu{display:none}";
    u = document.createElement("style");
    u.type = "text/css";
    u.styleSheet ? u.styleSheet.cssText = e : u.appendChild(document.createTextNode(e));
    n("head")[0].appendChild(u)
}(jQuery, this),
function() {
    ...
    i = function() {
        ...
        n.prototype.results_option_build = function(n) {
            ...
            f = u.length; i < f; i++)
                t = u[i],
                r += t.group ? this.result_add_group(t) : this.result_add_option(t),
                (n != null ? n.first : void 0) && (t.selected && this.is_multiple ? this.choice_build(t) : t.selected && !this.is_multiple && this.single_set_selected_text(t.text));
            return r
        }
        ,
        ...
        ,
        n.prototype.stringEscaped = function(n) {
            for (var i = [{
                val: "a",
                let: "(Á|À|Ã|Â|Ä|á|à|ã|â|ä|a|A)"
            }, {
                val: "e",
                let: "(É|È|Ê|Ë|é|è|ê|ë|e|E)"
            }, {
                val: "i",
                let: "(Í|Ì|Î|Ï|í|ì|î|ï|i|I)"
            }, {
                val: "o",
                let: "(Ó|Ò|Õ|Ô|Ö|ó|ò|õ|ô|ö|o|O)"
            }, {
                val: "u",
                let: "(Ú|Ù|Û|Ü|ú|ù|û|ü|u|U)"
            }, {
                val: "c",
                let: "(Ç|ç|c|C)"
            }], t = 0; t < i.length; t++)
                n = n.replace(new RegExp(i[t].let,"g"), i[t].val),
                n = n.replace(new RegExp(i[t].val,"g"), i[t].let);
            return n
        }
        ,
        n.prototype.winnow_results = function() {
            var a, n, u, e, t, i, f, s, o, l, h, r, c;
            ...
            u = i.split(" "),
            _s = 0,
            _lens = r.length; _s < _lens; _s++)
                u[_s] = new RegExp(this.stringEscaped(r[_s]),"i");
            for (o = 0,
            l = h.length; o < l; o++)
                if (n = h[o],
                n.search_match = !1,
                ...
                t.active_options === 0 && t.search_match && (e += 1),
                t.active_options += 1),
                !(n.group && !this.group_search))) {
                    for (n.search_text = n.group ? n.label : n.html,
                    c = 0,
                    _s = 0,
                    _lens = u.length; _s < _lens; _s++)
                        n.search_match = this.search_string_match(n.search_text, u[_s]),
                        n.search_match && !n.group && (c += 1),
                        c == _lens ? e += 1 : n.search_match = null;
                    if (n.search_match) {
                        if (i.length)
                            for (_s = 0,
                            _lens = r.length; _s < _lens; _s++)
                                f = n.search_text.search(u[_s]),
                                s = n.search_text.substr(0, f + r[_s].length) + "<\/em>" + n.search_text.substr(f + r[_s].length),
                                n.search_text = s.substr(0, f) + "<em>" + s.substr(f);
                        t != null && (t.group_match = !0)
                    } else
                        n.group_array_index != null && this.results_data[n.group_array_index].search_match && (n.search_match = !0)
                }
            return this.result_clear_highlight(),
            e < 1 && i.length ? (this.update_results_content(""),
        ...
        ,
        n.default_multiple_text = "",
        n.default_single_text = "",
        n.default_no_result_text = "Nenhum item contém",
        n
    }();
    n = jQuery;
    n.fn.extend({
    ...
    r = function(i) {
        ...
        r.prototype.search_field_disabled = function() {
            return (this.is_disabled = this.form_field_jq[0].disabled || n(this.form_field_jq[0]).is("[readonly]"),
            this.is_disabled || this.is_readOnly) ? (this.container.addClass("chosen-disabled"),
...
.call(this),
function(n, t) {
    typeof define == "function" && define.amd ? define(t) : typeof exports == "object" ? module.exports = t() : n.VMasker = t()
}(this, function() {
    function h(n) {
        var t = n.name
          , i = $(n);
        return n.className.indexOf("cpfcnpj") !== -1 && i.closest(".modal-dialog").length >= 1 ? t == "Inscricao" ? !0 : !1 : !1
    }
    var i = "9", r = "A", u = "S", f = [9, 16, 17, 18, 36, 37, 38, 39, 40, 91, 92, 93], s = function(n) {
        ...
        n.moneyPrecision = n.zeroCents ? 0 : n.precision,
        n
    }, o = function(n, t, f) {
        for (; t < n.length; t++)
            (n[t] === i || n[t] === r || n[t] === u) && (n[t] = f);
        return n
    }, t = function(n) {
        this.elements = n
    }, n;
    return t.prototype.unbindElementToMask = function() {
        for (var n = 0, t = this.elements.length; n < t; n++)
            this.elements[n].lastOutput = "",
            this.elements[n].onkeyup = !1,
            this.elements[n].onkeydown = !1,
            this.elements[n].value.length && (this.elements[n].value = this.elements[n].value.replace(/\D/g, ""))
    }
    ,
    t.prototype.bindElementToMask = function(t) {
        for (var i = this, u = function(r) {
            r = r || window.event;
            var u = r.target || r.srcElement;
            s(r.keyCode) && setTimeout(function() {
                i.opts.lastOutput = u.lastOutput;
                h(u) && i.opts.pattern == "999.999.999-99" && u.value.length == 18 ? (i.opts.pattern = "99.999.999/9999-99",
                u.value = n[t](u.value, i.opts)) : u.value = n[t](u.value, i.opts);
                u.lastOutput = u.value;
                u.setSelectionRange && i.opts.suffixUnit && u.setSelectionRange(u.value.length, u.value.length - i.opts.suffixUnit.length)
            }, 0)
        }, r = 0, f = this.elements.length; r < f; r++)
            this.elements[r].lastOutput = "",
            this.elements[r].onkeyup = u,
            this.elements[r].value.length && (this.elements[r].value = n[t](this.elements[r].value, this.opts))
    }
    ,
    t.prototype.maskMoney = function(n) {
        this.opts = e(n);
        this.bindElementToMask("toMoney")
    }
    ,
    t.prototype.maskNumber = function() {
        this.opts = {};
        this.bindElementToMask("toNumber")
    }
    ,
    t.prototype.maskAlphaNum = function() {
        this.opts = {};
        this.bindElementToMask("toAlphaNumeric")
    }
    ,
    t.prototype.maskPattern = function(n) {
        this.opts = {
            pattern: n
        };
    ...
    n = function(n) {
        ...
        var i = "length"in n ? n.length ? n : [] : [n];
        return new t(i)
    }
    ,
    n.toMoney = function(n, t) {
        var u, s, l;
        if (t = e(t),
        t.zeroCents) {
            t.lastOutput = t.lastOutput || "";
            var a = "(" + t.separator + "[0]{0," + t.precision + "})"
              , v = new RegExp(a,"g")
              , y = n.toString().replace(/[\D]/g, "").length || 0
              , p = t.lastOutput.toString().replace(/[\D]/g, "").length || 0;
            n = n.toString().replace(v, "");
            y < p && (n = n.slice(0, n.length - 1))
        }
        var f = n.toString().replace(/[\D]/g, "")
          , w = new RegExp("^(0|\\" + t.delimiter + ")")
          , b = new RegExp("(\\" + t.separator + ")$")
          , i = f.substr(0, f.length - t.moneyPrecision)
          , r = i.substr(0, i.length % 3)
          , o = new Array(t.precision + 1).join("0");
        for (i = i.substr(i.length % 3, i.length),
        u = 0,
        s = i.length; u < s; u++)
            u % 3 == 0 && (r += t.delimiter),
            r += i[u];
        ...
        r = r.length ? r : "0",
        !t.zeroCents) {
            var k = f.length - t.precision
              , h = f.substr(k, t.precision)
              , c = h.length
              , d = t.precision > c ? t.precision : c;
            o = (o + h).slice(-d)
        }
        return l = t.unit + r + t.separator + o + t.suffixUnit,
        l.replace(b, "")
    }
    ,
    n.toPattern = function(n, t) {
        for (var l = typeof t == "object" ? t.pattern : t, a = l.replace(/\W/g, ""), e = l.split(""), s = n.toString().replace(/\W/g, ""), v = s.replace(/\W/g, ""), h = 0, y = e.length, c = typeof t == "object" ? t.placeholder : undefined, f = 0; f < y; f++)
            if (h >= s.length) {
                if (a.length == v.length)
                    return e.join("");
                if (c !== undefined && a.length > v.length)
                    return o(e, f, c).join("");
                break
            } else if (e[f] === i && s[h].match(/[0-9]/) || e[f] === r && s[h].match(/[a-zA-Z]/) || e[f] === u && s[h].match(/[0-9a-zA-Z]/))
                e[f] = s[h++];
            else if (e[f] === i || e[f] === r || e[f] === u)
                return c !== undefined ? o(e, f, c).join("") : e.slice(0, f).join("");
        return e.join("").substr(0, f)
    }
    ,
...
$(document).ready(function() {
    $(".pnlCollapse").on("click", "h1", function() {
        var n = $(this).parent();
        n.hasClass("emitente") ? n.toggleClass("semi-aberto") : n.toggleClass("aberto");
        n.find(".retratil").slideToggle()
    });
    $(".alert").on("click", ".close", function() {
        $(this).closest(".alert").slideUp()
    });
    $(".menu-suspenso-tabela .icone-trigger").popover({
        html: !0,
        placement: "left",
        trigger: "manual",
        content: function() {
    ...
    $(".menu-suspenso-tabela").on("click", ".icone-trigger", function() {
        $(".menu-suspenso-tabela .icone-trigger").not(this).each(function() {
            $(this).popover("hide");
            $(this).closest("tr").removeClass("selecionada")
        });
        $(this).parent().find(".popover").length == 0 ? ($(this).closest("tr").addClass("selecionada"),
        $(this).popover("show")) : ($(this).popover("hide"),
        $(this).closest("tr").removeClass("selecionada"))
    });
    $(".menu-suspenso-tabela").on("click", ".list-group-item", function() {
        $(this).closest(".menu-suspenso-tabela").find(".icone-trigger").popover("hide")
    });
    $("body").on("click", function(n) {
        $(".menu-suspenso-tabela .icone-trigger").each(function() {
            $(this).is(n.target) || $(this).has(n.target).length !== 0 || $(".popover").has(n.target).length !== 0 || ($(this).popover("hide"),
            $(this).closest("tr").removeClass("selecionada"))
        })
    });
    $("form.formdps").on("click", ".btn.has-spin", function() {
        var t = $(this), n;
        return $("#hdfAlertaSobreposicao").length == 0 ? (n = t.closest("form"),
        n != null && n.submit()) : $.confirm({
            icon: "fa fa-warning",
            title: "Confirmação",
            content: "Os dados já informados para os passos subsequentes serão perdidos. Deseja continuar?",
            animation: "opacity",
            closeAnimation: "opacity",
            animateFromElement: !1,
            columnClass: "medium",
    ...
    $("span.monetario").each(function() {
        var n = $(this).text()
          , t = $(this).hasClass("aceita-zero");
        $(this).text("R$ " + FormataValorMonetario(n, t))
    });
    $("span.percentual").each(function() {
        var n = $(this).text();
        n.indexOf(",") == -1 && (n += ",00");
        $(this).text(FormataValorPercentual(n) + "%")
    });
    FuncoesBasicas()
})
