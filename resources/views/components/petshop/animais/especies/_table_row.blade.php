<tr>
    <td class="text-center">
        <form class='d-flex align-items-center gap-1' action="{{ route('animais.especies.destroy', $item->id) }}" method="post"
            id="form-{{ $item->id }}">
            @method('delete')
            @csrf
            ...
            <a
                ...
                title="Editar Pet"
                href="{{ route('animais.especies.edit', [$item->id, 'page' => request()->query('page', 1)]) }}">
                <img
                    height="26"
                    width="26"
